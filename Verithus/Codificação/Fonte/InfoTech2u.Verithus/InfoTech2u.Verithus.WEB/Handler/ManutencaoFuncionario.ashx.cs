...
namespace InfoTech2u.Verithus.WEB.Handler
{
    ...
    /// </summary>
    public class ManutencaoFuncionario : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (Acao == "Inclusao")
            {
                if (!String.IsNullOrWhiteSpace(CodigoUsuarioCadastro))
                    objFuncionario.CodigoUsuarioCadastro = Convert.ToInt32(context.Session["CodigoUsuario"].ToString());
                else
                    objFuncionario.CodigoUsuarioCadastro = Convert.ToInt32(context.Session["CodigoUsuario"].ToString());
