    ...
    jQuery('#btnIncluir').click(function (event) {
        if (ValidarFormulario()) {
            jQuery.ajax({
                type: "GET",
                crossDomain: true,
                url: "../../Handler/ManterCargo.ashx?Incluir",
                contentType: "json",
                cache: false,
                data: {
                    Metodo: 'Incluir',
                ...
                success: function (data) {
                    ...
                    Limpar();
                    if (tipos.length > 0) {
                        var row = '<tr id="' + tipos[0].CODIGO_TIPO_CARGO + '"><td>' + tipos[0].CODIGO_TIPO_CARGO + '</td><td>' + tipos[0].DESCRICAO + '</td><td class="centeralign"><a title="Excluir" href="javascript:Excluir(' + tipos[0].CODIGO_TIPO_CARGO + ');" class="deleterow"><i class="icon-trash"></i></a></td></tr>';
    ...
    function CarregarLista() {
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                var tipos = eval(data);
                for (x in tipos) {
                    var row = '<tr id="' + tipos[x].CodigoTipoCargo + '"><td>' + tipos[x].CodigoTipoCargo + '</td><td>' + tipos[x].Descricao + '</td><td class="centeralign"><a title="Excluir" href="javascript:Excluir(' + tipos[x].CodigoTipoCargo + ');" class="deleterow"><i class="icon-trash"></i></a></td></tr>';
                    jQuery('tbody').append(row);
                }
                FormatarGrid();
            },
